setwd('B:/MSA/Fall Term 1/Linear Algebra/MSA2018Photos/MSA2018Photos/')
Names = read.csv('LastNames.csv',header=F,colClasses = c("character"))
...
for (n in ind){
  value =  u[,n]
  im=as.cimg(data.frame(x=pics$x,y=pics$y,value))
  plot(im,xlim=c(0,400), main=paste("Singular Component",n))
}
#################################################################################
#################################################################################
